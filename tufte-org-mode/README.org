...
Side notes can be italicized
{{{sidenote(\it Sidenotes can be italicized\, like this.,0pt)}}}
by starting the ~text~ argument with the latex ~\it~ command.
To have marginnotes italicized by default, modify the macro definition to include ~\it~ before ~$1~.

#+begin_src org
,#+MACRO: sidenote \sidenote[$2][$3]{\it $1}
#+end_src
...
Like sidenotes, margin notes can also be italicized by starting the ~text~ argument with the latex ~\it~ command.
{{{marginnote(\it Marginnotes can be italicized\, like this.,0pt)}}}
To have marginnotes italicized by default, modify the macro definition to include ~\it~ before ~$1~.
